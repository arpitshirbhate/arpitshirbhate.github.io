...
export const CLIConsole = ({ isOpen, onClose }) => {
  ...
  const handleCommand = (cmd) => {
    ...
    switch (lowerCmd) {
      ...
      default:
        newHistory.push(
          `Command not found: "${cmd}". Type "help" for a list of available actions.`,
        );
    }
        ...
        >
          <motion.div
            ...
            onClick={(e) => {
              ...
              if (e.target === e.currentTarget || e.target.closest(".flex-1")) {
                inputRef.current?.focus();
              }
            }}
          >
              ...
              {history.map((line, index) => {
                ...
                if (line.includes("arpit@portfolio:~$")) {
                  return (
                    <div
                      key={index}
                      className="text-primary font-bold mb-1.5 flex items-center gap-1.5"
                    >
                ...
                if (
                  ...
                  if (line.startsWith("Welcome")) {
                    colorClass =
                      "text-primary font-bold text-sm sm:text-base mb-1";
                ...
                return (
                  <div
                    key={index}
                    className="whitespace-pre-wrap text-zinc-400 pl-4 py-0.5 border-l border-zinc-900 hover:border-zinc-800 transition-colors"
                  >
                    {line}
                  </div>
                );
